...
Write-Output "$(Get-Date) Begin: Building Visual Studio 2019 Offline ISO..."

Write-Output "`n"
Write-Output "$(Get-Date) Step 1: Checking prerequisites..."

$cdpacker = "$PSScriptRoot\cdimage.exe"
$vsInstaller = "$PSScriptRoot\vs_community.exe"
$autorunInf = "$PSScriptRoot\autorun.inf"

if (-Not ((Test-Path $cdpacker) -and (Test-Path $vsInstaller))) {    
    Critical-Error "Visual Studio 2019 offline installer not exist or CDIMAGE.exe not exist, building cannot continue! "    
} else {
    Write-Output "Visual Studio 2019 offline installer and CDIMAGE.exe check complete. "
}

if (-Not (Test-Path $autorunInf)) {
    Write-Warning "$autorunInf not found, the Visual Studio 2019 iso will have no autorun information (say icon etc). "
} else {
...
Write-Output "`n"
Write-Output "$(Get-Date) Step 3: Downloading Visual Studio 2019 offline files..."
...
Write-Output "`n"
Write-Output "$(Get-Date) Step 4: Building Visual Studio 2019 installation iso..."

# https://stackoverflow.com/questions/2095088/error-when-calling-3rd-party-executable-from-powershell-when-using-an-ide
& $cdpacker -n -m -d -lVS2019Com $isoFolder $isoFile 2>&1 | %{ "$_" }
